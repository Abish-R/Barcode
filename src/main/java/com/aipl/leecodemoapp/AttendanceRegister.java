...
public class AttendanceRegister extends AppCompatActivity implements View.OnClickListener{
    ...
    Button cam_image, save, sync,gps_loc,report_invoker;
...
//        else
        if(photo_taken)
            ...
            if(gps.canGetLocation())
                DoubleButtonAlert("loc","GPS is enabled. Do you want to go settings to turn off ?","Settings","Cancel");
            else {
                super.onBackPressed();
            }
            //super.onBackPressed();
...
//    }
    ...
    private void initializeViews() {
        cam_image = (Button) findViewById(R.id.cam_image);
        save = (Button) findViewById(R.id.save);
        sync = (Button) findViewById(R.id.sync);
        sync.setVisibility(View.GONE);
        report_invoker = (Button) findViewById(R.id.report_invoker);
        gps_loc = (Button) findViewById(R.id.gps_loc);
        ...
        //in_checkBox = (CheckBox) findViewById(R.id.in_checkBox);
        //out_checkBox = (CheckBox) findViewById(R.id.out_checkBox);
        gps_position = (TextView) findViewById(R.id.gps_position);

        cam_image.setOnClickListener(this);
        save.setOnClickListener(this);
        sync.setOnClickListener(this);
        gps_loc.setOnClickListener(this);
        report_invoker.setOnClickListener(this);
        //in_checkBox.setOnCheckedChangeListener(this);
        //out_checkBox.setOnCheckedChangeListener(this);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.gps_loc:
                againAskingGPS();
                break;
            case R.id.report_invoker:
                finish();
                Intent intent = new Intent(AttendanceRegister.this,MainActivity_new.class);
                intent.putExtra("mainpage_hide",1);
                startActivity(intent);
                break;
            case R.id.sync:
                //Toast.makeText(AttendanceRegister.this, "Sync Under Development", Toast.LENGTH_SHORT).show();
                resetPage();
                //syncPerform();
                break;
        }
...
////        int auto_time_check=android.provider.Settings.Global.getInt(getContentResolver(), android.provider.Settings.Global.AUTO_TIME, 0);
////        int auto_time_zone_check=android.provider.Settings.Global.getInt(getContentResolver(), android.provider.Settings.Global.AUTO_TIME_ZONE, 0);
////        if(auto_time_check==1 && auto_time_zone_check==1){
//            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
//            cc = Calendar.getInstance();
//        switch (group.getId()) {
//            case R.id.in_checkBox:
//                //dbdth.checkBeforeInsertion("");
//                if (checked && dbdth.checkInTimePunchedOrNot(sdf1.format(c.getTime()))==0) {
//                    checkedstate = "intime";
...
//                    if (checkedstate.equals("outtime"))
//                        Log.d("Checked State", checkedstate);
//                    else{
//                        if(checkedstate.equals(""))
//                            Toast.makeText(this,"In-Time already punched.",Toast.LENGTH_SHORT).show();
//                        checkedstate = "";
//                    }
//                    in_time.setText("");
//                    out_time.setText("");
...
//                break;
//            case R.id.out_checkBox:
//                if (checked && dbdth.checkOutTimePunchedOrNot(sdf1.format(c.getTime()))==100) {
//                    out_checkBox.setChecked(false);
//                    in_checkBox.setChecked(false);
//                    Toast.makeText(this,"In-Time not punched yet.",Toast.LENGTH_SHORT).show();
//                    checkedstate = "";
//                    in_time.setText("");
//                    out_time.setText("");
//                }
//                else if (checked && dbdth.checkOutTimePunchedOrNot(sdf1.format(c.getTime()))>0) {
//                    checkedstate = "outtime";
//                    in_checkBox.setChecked(false);
//                    out_time.setText(sdf.format(cc.getTime()));
//                    in_time.setText("");
//                } else {
//                    out_checkBox.setChecked(false);
//                    if (checkedstate.equals("intime"))
//                        Log.d("Checked State", checkedstate);
//                    else {
//                        if(checkedstate.equals(""))
//                            Toast.makeText(this,"Out-Time Already punched.",Toast.LENGTH_SHORT).show();
//                        checkedstate = "";
//                    }
//                    in_time.setText("");
//                    out_time.setText("");
...
//    }
    ...
    public void syncPerform(){
        ...
        if(!ci.isOnline()) {
            ...
            for (GetSetAttandance img : image_list) {
                count++;
                int i=0;
...
//            SingleButtonAlert("Attendance", "You should capture your picture before Saving", "Ok", "");
            ...
            if (getLatLong()==1) {
                ...
                if(dbdth.addAttendance(new GetSetAttandance("abcd123","abish.r01@gmail.com","9688910689",gdi.getDeviceId(),
                    ...
                    SingleButtonAlert("Attendance", "In Time successfully completed.", "Ok", "nextpage");
                    }
                photo_taken = false;
                in_time.setText("");
                resetPage();
            ...
            if (getLatLong()==1) {
                ...
                if(dbdth.updateAttendance(new GetSetAttandance(intime, pic_name, latitude + "", longitude + "",
                        gps_address, "N", attend_date,"N"))==0) {
                    SingleButtonAlert("Attendance", "You can't put out time before in time", "Ok", "");
                }
                else {
                    sciis.saveImage("EAttendance", pic_name, cam_img);
                    DoubleButtonAlert("wait", "Attendance process completed.\nNow you can proceed to Sync", "Ok", "Skip");
                }
                photo_taken = false;
                in_time.setText("");
                resetPage();
    ...
    public int notUploadCount(){
        List<GetSetAttandance> attend_data=dbdth.getAttendanceDataNotUploaded();
    ...
    private int bringTableDataToSync(){
        ...
        for (GetSetAttandance gs : attend_data) {
...
//                new AsyncSyncData(AttendanceRegister.this).execute(user_id,email,mobile,device_id,in_time,out_time,in_pic_name,
...
//        out_checkBox.setChecked(false);
    ...
    public void SingleButtonAlert(String title,String message,String but_txt,final String _state){
        ...
        alertDialogBuilder.setPositiveButton(but_txt, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                if (_state.equals("nextpage")){
                    finish();
                    Intent intent = new Intent(AttendanceRegister.this,MainActivity_new.class);
                    intent.putExtra("mainpage_hide",0);
                    startActivity(intent);
                }
    ...
    public void DoubleButtonAlert(final String text, String msg, String btn1, String btn2){
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(AttendanceRegister.this);
        //alertDialog.setTitle("GPS settings");
        alertDialog.setMessage(msg);
        alertDialog.setPositiveButton(btn1, new DialogInterface.OnClickListener() {
        ...
        alertDialog.setNegativeButton(btn2, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                dialog.cancel();
                if(text.equals("loc"))
                    finish();
